...
def train(config, workdir):
    ...
    if config.training.sde.lower() == 'vpsde':
        sde = methods.VPSDE(config=config, beta_min=config.model.beta_min, beta_max=config.model.beta_max,
                            N=config.model.num_scales)
        sampling_eps = 1e-3
    elif config.training.sde.lower() == 'subvpsde':
        sde = methods.subVPSDE(config=config, beta_min=config.model.beta_min, beta_max=config.model.beta_max,
                               N=config.model.num_scales)
        sampling_eps = 1e-3
    elif config.training.sde.lower() == 'vesde':
        sde = methods.VESDE(config=config, sigma_min=config.model.sigma_min, sigma_max=config.model.sigma_max,
                            N=config.model.num_scales)
        sampling_eps = 1e-5
    ...
    for ckpt in range(begin_ckpt, config.eval.end_ckpt + 1):
        ...
        if config.training.sde == 'poisson':
            ckpt_filename = os.path.join(checkpoint_dir,
                                         "checkpoint_{}.pth".format(ckpt * config.training.snapshot_freq))
        ...
        if config.eval.enable_sampling:
            ...
            for r in range(num_sampling_rounds):
                ...
                samples_torch = samples_torch.view(-1, config.data.num_channels, config.data.image_size,
